...
*/
	...
	public int insert(Responsable responsable ,Connection con)throws ResponsableException {
		...
		try
		{
			ps = con.prepareStatement("insert into RESPONSABLE( IDRESPONSABLE, NOMBRE, APELLIDO, TELEFONO, EMAIL, DOMICILIO, NRODOCUMENTO, TIPODOCUMENTO, FAX) values (?, ?, ?, ?, ?, ?, ?, ?, ?)");
				ps.setLong(1,responsable.getIdresponsable());
				ps.setString(2,responsable.getNombre());
				ps.setString(3,responsable.getApellido());
				ps.setString(4,responsable.getTelefono());
				ps.setString(5,responsable.getEmail());
				ps.setLong(6,responsable.getDomicilio());
				ps.setInt(7,responsable.getNrodocumento());
				ps.setLong(8,responsable.getTipodocumento());
				ps.setString(9,responsable.getFax());
